 ...
 */
const calcCharsetLengthWith =
  (charsets: Charset[]): Function =>
  (string: string) =>
    charsets.reduce(
      (length, charset) =>
 ...
 * @type {Function}
 */
const calcCharsetLength: Function = calcCharsetLengthWith(stdCharsets);
 ...
 * @returns {number}        [the calculated entropy.
 */
export const passwordEntropy = (password: string): number => {
  return password
    ? calcEntropy(calcCharsetLength(password), password.length)
    : 0;
};
